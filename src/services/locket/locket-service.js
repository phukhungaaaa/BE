...
const postImage = async (userId, idToken, image, caption, textColor) => {
    try {
                ...
                overlays: [
                    {
                        data: {
                            text: caption,
                            text_color: textColor, // Thêm tham số màu sắc cho caption
                            type: "standard",
                            max_lines: {
                                "@type":
...
const uploadVideoToFirebaseStorage = async (userId, idToken, video) => {
    try {
        ...
        // Bước 2: Tải dữ liệu video lên thông qua URL resumable trả về từ bước 1
        ...
        const videoDownloadUrl = `https://firebasestorage.googleapis.com/v0/b/locket-video/o/users%2F${userId}%2Fmoments%2Fvideos%2F${videoName}`;
        const getHeaders = {
            authorization: `Bearer ${idToken}`,
        };

        const getResponse = await fetch(videoDownloadUrl, {
            method: "GET",
            headers: getHeaders,
        });
        ...
        if (!getResponse.ok) {
            throw new Error(
                `Failed to get video download token: ${getResponse.statusText}`
            );
        }

        const downloadToken = (await getResponse.json()).downloadTokens;
        return `${videoDownloadUrl}?alt=media&token=${downloadToken}`;
    } catch (error) {
...
const postVideo = async (userId, idToken, video, caption, textColor) => {
    try {
